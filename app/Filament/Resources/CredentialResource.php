                ...
                Section::make(__('credential.section.credential'))
                    ->schema([
                        ...
                        Select::make('provider')
                            ...
                            ->options([
                                'github' => __('credential.field.provider_github'),
                                'gitlab' => __('credential.field.provider_gitlab'),
                            ])
                            ->default('github')
                            ->required()
                            ->live()
                        ...
                        TextInput::make('base_url')
                            ->label(__('credential.field.base_url'))
                            ->placeholder(__('credential.field.base_url_placeholder'))
                            ->url()
                            ->maxLength(255)
                            ->columnSpanFull()
                            ->helperText(__('credential.field.base_url_helper'))
                            ->visible(fn (Get $get): bool => $get('provider') === 'gitlab'),
                        TextInput::make('token')
                            ...
                            ->helperText(fn (Get $get): HtmlString => new HtmlString(
                                $get('provider') === 'gitlab'
                                    ? __('credential.field.token_helper_gitlab')
                                    : __('credential.field.token_helper')
                            ))
                            ->visibleOn('create'),
            ...
            ->columns([
                TextColumn::make('name')
                    ...
                    ->description(function (Credential $record): HtmlString {
                        ...
                        // Provider icon with label
                        $isGitLab = $record->provider === 'gitlab';
                        $providerLabel = $isGitLab
                            ? __('credential.field.provider_gitlab')
                            : __('credential.field.provider_github');
                        ...
                        $gitlabIcon = '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M23.955 13.587l-1.342-4.135-2.664-8.189a.455.455 0 0 0-.867 0L16.418 9.45H7.582L4.918 1.263a.455.455 0 0 0-.867 0L1.386 9.45.044 13.587a.924.924 0 0 0 .331 1.023L12 23.054l11.625-8.443a.92.92 0 0 0 .33-1.024"/></svg>';
                        $providerIcon = $isGitLab ? $gitlabIcon : $githubIcon;
                        $parts[] = "<span style=\"display:inline-flex;align-items:center;gap:4px\">{$providerIcon} {$providerLabel}</span>";
                ...
                Section::make(__('credential.section.credential'))
                    ...
                    ->schema([
                        ...
                        TextEntry::make('base_url')
                            ...
                            ->visible(fn (Credential $record): bool => $record->provider === 'gitlab'),
